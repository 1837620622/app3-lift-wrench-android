...
- 主界面是现场工作台，不承载曲线阅读，只显示连接状态、现场主读数和进入曲线大屏的入口。
- 曲线大屏是独立页面，要能清楚看到扭矩、顶升力、压力随角度逐步增大并接近平衡点的过程。
...
- Android Wi-Fi 扫描、手动连接引导、运行时权限引导、CSV/Excel 导出、历史回放控制和正式标定参数管理仍需继续实现。
...
- `App3Android/docs/真机USB调试方案.md`：真实安卓手机 USB 调试方案。
